...
const vertexShader = `
    ...
    void main() {
        float scale = 0.8;
        float roughness = 0.4;
        float factor = snoise(position * scale) * roughness;
        factor = mapRangeLinear(factor, -0.5, 0.4, 0.0, 1.0, 8.0);

        vec3 newPosition = position + normal * factor;
...
const fragmentShader = `
    ...
    vec3 colorRamp(vec3 startColor, vec3 endColor, float t) {
        return mix(startColor, endColor, t);
    }

    void main() {
        // vec3 startColor = vec3(0.6, 1.0, 0.2);
        // vec3 endColor = vec3(1.0, 0.02, 0.002);
        vec3 startColor = vec3(0.9, 0.9, 0.9);
        vec3 endColor = vec3(0.2, 0.2, 0.2);
        
        vec3 interpolatedColor = colorRamp(startColor, endColor, vDisplacement);
        gl_FragColor = vec4(interpolatedColor + vec3(0.05), 1.0);
    }
`;
